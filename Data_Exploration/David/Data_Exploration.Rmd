...
```{r}
# Boxplots
boxplot(NSW_data$Diff1, NSW_data$Diff32, NSW_df$lm1_diff, NSW_df$lm2_diff)

# Caluculate five number summaries
fivenum(NSW_data$Diff1)
fivenum(NSW_data$Diff32)
fivenum(NSW_df$lm1_diff)
fivenum(NSW_df$lm2_diff)
```
...
#ggplot(NSW_df, aes(x = time(DATETIME), y = TOTALDEMAND, group = time(DATETIME))) + geom_boxplot()
